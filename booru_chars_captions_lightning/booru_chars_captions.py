...
class BooruCharsCaptionsDataset(IterableDataset):
  ...
  def __init__(self, file_ids: Iterable[BooruFileId], get_cursor: Callable[[], Cursor]) -> None:
    super(BooruCharsCaptionsDataset).__init__()
    self.file_ids = file_ids
    self.get_cursor = get_cursor
  
  def __iter__(self) -> Iterator[CaptionRecord]:
...
class BooruCharsCaptions(LightningDataModule):
  ...
  def setup(self, stage: Optional[str] = None) -> None:
    ...
    if stage == "fit" or stage is None:
      ...
      self.train_dataset = BooruCharsCaptionsDataset(file_ids=map(enumeration_to_value, training), get_cursor=get_cursor)
      self.validation_dataset = BooruCharsCaptionsDataset(file_ids=map(enumeration_to_value, validation), get_cursor=get_cursor)
    ...
    if stage == "test" or stage is None:
      ...
      file_id_dtos: Iterator[BooruFileId] = file_ids_to_dtos(file_ids)
      get_cursor = lambda: conn.cursor()
      self.test_dataset = BooruCharsCaptionsDataset(file_ids=file_id_dtos, get_cursor=get_cursor)
